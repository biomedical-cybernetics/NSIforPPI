...
for i = 1:length(networks)
    ...
    % check output
    temp = [networks{i} '_linkrem10noconn_CHv2_L2_L3_linkpred_AUCPR.TEMP'];
    output = ['../network_similarities/CH_L2_L3/results/' networks{i} '_linkrem10noconn_CHv2_L2_L3_linkpred_AUC_PR.mat'];
    ...
    for m = 1:length(methods)
        eval([methods{m} '_auc_pr = zeros(iters,1);'])
        vartosave = [vartosave {[methods{m} '_auc_pr']}];
    end
    ...
    if ~exist(check_output,'file')
        ...
        for j = 1:iters
            ...
            for m = 1:length(methods)
                ...
                eval([method '_auc_pr(j) = linkpred_evaluation(x, matrices{j}, ranking(idx_A,:));'])
            end
...
function [auc_pr] = linkpred_evaluation(x, x_spars, ranking)
...
%%% OUTPUT %%%
% auc_pr - area under curve precision-recall
