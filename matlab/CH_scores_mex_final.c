 ...
 * mex C:\ProgramData\MATLAB\SupportPackages\R2023a_1\3P.instrset\mingw_w64.instrset\lib\gcc\x86_64-w64-mingw32\6.3.0\libgomp.a CH_scores_mex_final.c CFLAGS='$CFLAGS -fopenmp' LDFLAGS='$LDFLAGS -fopenmp'
 *
 * Compile in Linux:
 ...
 *_______________________________________________________________________*/
 ...
 void compute_scores_from_source_node(mwIndex *ir, mwIndex *jc, mwSize N,
         ...
         mxDouble *models, mwSize M, mxDouble **scores_cell, mxDouble *nd_connections)
 {
     mwIndex i, j, l, m, nd, n, p, paths_idx, nodes_lc_size, length;
     mxDouble deg_gmean, deg_i_gmean, deg_i1_gmean, deg_e1_gmean, pow_exp, deg_e1_gmean_v3a;
     ...
     for (l=0; l<L; l++)
     {
         ...
         for (nd=0; nd<N; nd++)
         {
             ...
             {   
                 ...
                 melcl = fmax(melcl, 1.0 / nodes_lc_size); /* avoid melcl to be zero*/
                 ...
                 for (p=0; p<P[paths_idx]; p++)
                 {
                     deg_gmean = 1;      /* for RA: geometric mean of degree of intermediate nodes */
                     deg_i_gmean = 1;    /* for CH1: geometric mean of internal degree of intermediate nodes */
                     deg_i1_gmean = 1;   /* for CH2: geometric mean of 1 + internal degree of intermediate nodes */
                     deg_e1_gmean = 1;   /* for CH3: geometric mean of 1 + external degree of intermediate nodes */                     
                     deg_e1_gmean_v3a = 1;
                     for (i=0; i<length-1; i++)
                     {
                     ...
                     for (m=0; m<M; m++)
                     {
                         ...
                         {
                             scores_cell[m*L+l][ns*N+nd] += deg_i_gmean / deg_i1_gmean;
                         }
                         else if (models[m]==5) /* CH3.1 */
                         {
                             scores_cell[m*L+l][ns*N+nd] += deg_i1_gmean/deg_e1_gmean_v3a;
                         }
 ...
 void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
 {
     ...
     #pragma omp parallel if(T>1) num_threads(T) shared(ir, jc, N, lengths, L, length_max, length_to_idx, deg, models, M, scores_cell) private(n, l)
     {
         ...
         mxDouble *nd_connections = (mxDouble*) mxCalloc(N, sizeof(mxDouble));   /* number of connections of each node */
         mwIndex paths_idx;
         for (l=0; l<L; l++)
         {
             for (n=0; n<N; n++)
         ...
         for (n=0; n<N; n++)
         {
             compute_scores_from_source_node(ir, jc, N, lengths, L, length_max, length_to_idx,
                     paths_cell, P_MAX, P, path, in_path, n, deg, nodes_lc, deg_i, deg_e, models, M, scores_cell, nd_connections);
         }
