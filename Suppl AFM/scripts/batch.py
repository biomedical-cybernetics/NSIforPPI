...
def predict_structure(
    ...
    today = date.today().strftime('%Y%m%d') # NEW
    out_suffix = '_' + today + '_' + str(random_seed)[-6:] # NEW

    model_names = []
    num_models = len(model_runner_and_params) # NEW
    print('num models :{}'.format(num_models))
    for (model_name, model_runner, params) in model_runner_and_params:
        logger.info(f"Running {model_name}")
        model_names.append(model_name)
        model_out_name = model_name + out_suffix # NEW
        ...
        timings[f'process_features_{model_name}'] = time.time() - t_0 # NEW
        start = time.time()
        ...
        timings[f'predict_and_compile_{model_name}'] = prediction_time

        mean_plddt = np.mean(prediction_result["plddt"][:seq_len])
        mean_ptm = prediction_result["ptm"]
        ranking_confidences[model_name] = prediction_result['iptm+ptm'] # NEW
        if rank_by == "plddt":
        ...
        def _save_results(result, log_model_name, out_dir):
            # Get mean pLDDT confidence metric.
            plddt = np.mean(result["plddt"][:seq_len]) # NEW
            plddtss[log_model_name] = round(plddt, 2)
            ptm = 0
            if 'ptm' in result:
        ...
        _save_results(prediction_result, model_out_name, result_dir)
        # End of model runs
    ...
    elif rank_by == "intscore": # NEW
        print(ints)
        model_rank = np.array(list(ints.values())).argsort()[::-1] # NEW
    ...
    for n, key in enumerate(model_rank):
        ranked_order.append(model_names[key]) # NEW
    ...
    stats = {'plddts': plddtss, 'ptms': ptms, 'order': ranked_order}
    ...
    if num_models > 1:  #more than 1 model
      stats_output_path = os.path.join(result_dir, 'stats_all' + out_suffix + '.json')
    else: #only one model, use different model names to avoid overwriting same file
      stats_output_path = os.path.join(result_dir, 'stats_' + model_out_name + '.json')
    with open(stats_output_path, 'w') as f:
      f.write(json.dumps(stats, sort_keys=True, indent=4))

    timings_output_path = os.path.join(result_dir, 'timings.json') # NEW
    with open(timings_output_path, 'w') as f: # NEW
        f.write(json.dumps(timings, indent=4)) # NEW
...
def get_queries(
    ...
    if input_path.is_file():
        if input_path.suffix == ".csv" or input_path.suffix == ".tsv":
            ...
            queries = []
            queries.append((headers, sequences.upper().split(":"), None))
...
def process_multimer_features(
    ...
    for chain_id, chain_features in features_for_chain.items():
        #print('chain_features: {}'.format(chain_features))
...
def generate_input_feature(
    ...
    if is_complex and model_type == "AlphaFold2-ptm":
        ...
        if not is_complex:
            ...
            #print('features_for_chain: {}'.format(features_for_chain))
...
def run(
    ...
    for job_number, (raw_jobname, query_sequence, a3m_lines) in enumerate(queries):
        ...
        try:
            ...
            timings['features'] = time.time() - t_0 # NEW
...
def main():
    ...
    parser.add_argument(
        "--rank",
        help="rank models by auto, plddt, ptmscore or intscore",
        type=str,
        default="auto",
        choices=["auto", "plddt", "ptmscore", "multimer", "intscore"],
    )
    parser.add_argument(
        "--pair-mode",
