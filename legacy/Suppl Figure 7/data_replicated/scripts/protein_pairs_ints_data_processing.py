...
# average interface score (INTS) values for protein pairs in positive and negative sets,
...
def merge_sets_for_all_variants(path_to_ints_outputs):

	model_preset = ['AlphaFold2-multimer-v1', 'AlphaFold2-multimer-v2']
	pair_mode = ['unpaired+paired', 'unpaired', 'paired']
	...
	for m in model_preset:
		for p in pair_mode:
			...
			if m == 'AlphaFold2-multimer-v1':
				merged_filename = 'Yeast_avg_ints_ColabFold_AF2Mv1_{}.txt'.format(p)
			elif m == 'AlphaFold2-multimer-v2':
				merged_filename = 'Yeast_avg_ints_ColabFold_AF2Mv2_{}.txt'.format(p)
			...
			for d in listDir:
				...
				if 'Positive_set' in d:
					if len(path_to_stats_all_file) == 0:
						...
						metric = json.load(f)
						avg_ints_pos.append(round(np.mean([value for key, value in metric['interface score'].items()]), 3))
						f.close()
					...
					if len(path_to_stats_all_file) == 0:
						...
						f = open(path_to_stats_all_file[0])
						metric = json.load(f)
						avg_ints_neg.append(round(np.mean([value for key, value in metric['interface score'].items()]), 3))
						f.close()

			resDict = {'Protein Pairs Pos' : protein_pairs_pos_name, 'Avg INTS Pos' : avg_ints_pos, \
			'Protein Pairs Neg' : protein_pairs_neg_name, 'Avg INTS Neg' : avg_ints_neg}

			with open(path_to_ints_outputs + merged_filename, 'w') as f:
				for row in zip_longest(*([key] + value for key, value in resDict.items()), fillvalue = None):
					print(*row, sep = '\t', file=f)



path_to_ints_outputs = '..\\INTS_outputs\\'
merge_sets_for_all_variants(path_to_ints_outputs)
