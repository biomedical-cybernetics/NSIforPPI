function [x, ids] = create_matrix(id1, id2, weights, conncomp)
...
%
% conncomp - 1 if the connected components have to be computed,
%            0 otherwise
%
%%% OUTPUT %%%
...
if ~isempty(weights)
    if ~(isvector(weights) && length(weights)==length(id1) && isnumeric(weights))
        error('weights must be a vector of numbers with the same length as id1.')
    end
    weighted = 1;
else
    weighted = 0;
